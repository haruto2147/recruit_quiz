...
QuestionList CreateHomophoneExam()
{
	...
	for (int i = 0; i < quizCount; i++) {
		...
		//問題文を作成
		const vector<int> answer = CreateRandomIndices(count);
		string s = "「" + string(e.words[answer[correctNo - 1]].kanji) +
			"」の意味として正しい番号を選べ";
